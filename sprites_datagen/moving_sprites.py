...
class MovingSpriteDataset(Dataset):
    ...
    def __init__(self, spec):
        self._spec = spec
        self._generator = DistractorTemplateMovingSpritesGenerator(self._spec) ## gets the indexes of the agent and the target shape along with the distractors
    ...
    def __getitem__(self, item):
        ...
        #data_dict.images = (traj.images[:, None].astype(np.float32)) / (255.0/2) - 1.0

        data_dict.states = traj.states
        data_dict.shape_idxs = traj.shape_idxs
        data_dict.rewards = traj.rewards
        #data_dict.imgs = make_image_seq_strip([traj.images[None, :, None].repeat(3, axis=2).astype(np.float32)], sep_val=255.0).astype(np.uint8) # added this line
...
class MovingSpritesGenerator:
    ...
    def __init__(self, spec):
        self._spec = spec
        bounds = [[self._spec.obj_size/2, 1 - self._spec.obj_size/2]] * 2  ## bounds for the sprites to move in  ## each list telling us which points b/w 0 and 1 the sprite can move along each dimention, one list for one dimention
...
class TemplateMovingSpritesGenerator(MovingSpritesGenerator): # extends the MovingSpritesGenerator
...
class DistractorTemplateMovingSpritesGenerator(TemplateMovingSpritesGenerator): # extends the TemplateMovingSpritesGenerator
