...
class FollowReward(Reward): # New reward function
    """Returns reward proportional to the distance between agent and target."""
    NAME = 'follow'

    def __call__(self, trajectories, shapes):
        return 1-np.linalg.norm(trajectories[:, 0, :] - trajectories[:, 1, :], axis=1)/np.sqrt(2)