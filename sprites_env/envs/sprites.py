import gym
from gym.spaces import Box # specifies the range of values for each dimension in the space - used to define the observation_space and action_space
import numpy as np
import cv2
import os
...
class SpritesEnv(gym.Env):
    ...
    def __init__(self, follow=True, **kwarg):
        ...
        self.pos_bounds = [[self.obj_size/2, 1 - self.obj_size/2]] * 2  # for the x and y positions, the maximum coordinates (normalized to 1) where the object can be placed
    ...
    def _forward(self, state):
        ...
        # Enables shapes bouncing off of walls                                      # left to understand the dynames of the environment
    ...
    def forward(self, state):
        ...
        return state[:, :self._n_dim].copy(), state  # pos_state, state
    ...
    def reset(self):
        ...
        state = np.random.uniform(size=(self.n_distractors + 2, self._n_state)) # [shape_idx, 4] for all shapes, therefore the dimension of state is [n_total shapes, 4]
        ...
        return im / 255     # normalize the image to [0, 1] 
    ...
    def _render(self, trajectories, shapes):    # not sure what this function does
    ...
    def render(self, mode='rgb_array'):   # not sure what this function does
...
if __name__  == '__main__':
    ...
    obs = env.reset()
    cv2.imwrite("image_outputs/test_rl.png", 255 * np.expand_dims(obs, -1))
    obs, reward, done, info = env.step([0, 0])
    cv2.imwrite("image_outputs/test_rl_1.png", 255 * np.expand_dims(obs, -1))
